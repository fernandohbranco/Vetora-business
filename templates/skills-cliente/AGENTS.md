...
```
_memoria/          ← lida por TODAS as skills (contexto do negócio)
identidade/        ← lida por skills visuais (design guide + design system)
_estado/           ← escrita/lida pelo /ops (estado de workflows em andamento)
  sessao-atual.md  ← workflow ativo (criado pelo /ops, atualizado por fase)
  historico/       ← workflows concluídos ou cancelados
```
...
Regra que vale para **toda** skill que gera material:

| Tipo de material | Marca a aplicar | Fonte |
|---|---|---|
| **Entregável** que a VETORA entrega PARA o cliente (relatório, proposta, auditoria) | **VETORA** | container (`.vetora-container` → `identidade/` do container) |
| **Conteúdo** que o cliente PUBLICA (post, carrossel, site, anúncio) | **do cliente** | `identidade/` deste repo |

- O repo de cliente fica aninhado sob o **container VETORA**. Skills de entregável (`/relatorio`,
  `/relatorio-ads`) sobem na árvore de pastas até achar o marcador `.vetora-container` e leem a
  marca VETORA de lá. Sem o marcador (clone fora do container), caem em fallback gracioso: avisam e
  usam branding genérico, sem quebrar.
- O `identidade/` deste repo é a marca **do cliente** — correta para o que o cliente publica, errada
  para um relatório que a VETORA assina.
...
| `/relatorio` | Tipo + insumos + `preferencias.md` + marca VETORA (container) | `saidas/relatorios/<data>-<tipo>.md` + `.pdf` (marca VETORA) | `/email-profissional` (na entrega) |
| `/relatorio-ads` | CSVs exportados das plataformas + `empresa.md` | `marketing/campanhas/relatorios/<data>-relatorio.md` + `.pdf` | reaproveita resolver+export de `/relatorio` |
...
| `/frontend-design` | Brief + `design-guide.md` + `design-system` (opcional) + `empresa.md` | Plano de design (inline) + código HTML/CSS/JS ou componente | — |
...
| `/ops` | Pedido em linguagem natural + `estrategia.md` + `_estado/sessao-atual.md` | Plano de execução com fases + `_estado/sessao-atual.md` atualizado | Qualquer skill |
...
/relatorio ──────────────────────────── [C2] — entregável com marca VETORA
  ├─ resolve: .vetora-container (sobe a árvore) → marca + motor de export do container
  ├─ usa:     <container>/scripts/render-report-pdf.js + report-template.html
  └→ output:  saidas/relatorios/<data>-<tipo>.md + .pdf

/relatorio-ads ──────────────────────── [C2]
  └─ reaproveita: resolver de marca + motor de export de /relatorio
...
/frontend-design ────────────────────── [C2]
  ├─ lê: identidade/design-guide.md + MASTER.md (opcional)
  └→ output: plano de design (inline) + código
...
/ops ────────────────────────────────── [C3] — Workflow Engine
  ├─ lê:   _estado/sessao-atual.md (retoma workflow pausado)
  ├─ cria: _estado/sessao-atual.md (plano de execução com fases)
  ├─ move: sessao-atual.md → _estado/historico/ (ao concluir)
  └─ chama: qualquer skill na ordem do plano
...
| `clones/design-ui-skills/` | mcp-builder, canvas-design, theme-factory, brand-guidelines, webapp-testing | Média |
